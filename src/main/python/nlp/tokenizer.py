...
class WordTokenizer(object):
  ...
  """
    ...
    3. remove stopwords
    4. lemmatization
    @INPUT:  document, if_use_stopwords
    @OUTPUT: tokens
  """
  def tokenize(self, document, stopwords=True):
    if (type(document) != unicode):
      raise UnicodeDecodeError("document should be decoded to utf8")
    tokens = []
    ...
    for sent in sent_tokenize(document):
      # 2. 
      for token, tag in pos_tag(self.word_tokenize(sent)):
